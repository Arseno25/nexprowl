...
**Nothing to configure.** NexProwl is distributed as source plus release
artifacts only:

| Channel | Setup needed |
|---|---|
| `go install` | none — the module path is the whole mechanism |
| Clone and `make install` | none |
| Release archives, `.deb`, `.rpm` | none — GoReleaser attaches them to the tag |
| Docker | none — users build the `Dockerfile` themselves |

There is no Homebrew tap, no APT or DNF repository, and no container registry
push, so there is no second repository to create, no GPG signing key to hold,
and no GitHub Pages site to publish. That is a deliberate choice: every hosted
package channel adds a long-lived credential and a piece of infrastructure that
has to keep working, and building from source is already a one-command
operation.

If a hosted channel is added later, it belongs in
[`packaging/README.md`](../packaging/README.md), which tracks what each one
would require.
